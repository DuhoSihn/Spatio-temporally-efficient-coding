function [ Z2, E2, Z1, E1, Z0, oZ1, oZ2 ] = STE_pred_sgm( sX, io_f, W_f, b_f, varargin )
...
dist_c_v = nan( nH, 1 );
for h = 1 : nH
    dist_c_v( h, 1 ) = sqrt( nZ( h ) ) / 2;% distance between the center and a vertex of the unit hypercube
end
...
for h = 1 : nH + 1
    if h == 1
        E_curr{ h, 1 } = zeros( size( sX( :, :, 1 ) ) );
    else
        E_curr{ h, 1 } = zeros( miniBatchSize, nZ( h - 1 ) );
    end
end
...
for tt = 1 : nPred
    ...
    [ t_Z, t_oZ ] = STE_f_sgm( Z_input, io_f, W_f, b_f );
...
for h = 1 : nH + 1
    E2{ h, 1 } = [ E1{ h, 1 }( miniBatchSize + 1 : end , : ); NaN( miniBatchSize, size( E1{ h, 1 },2  ) ) ];
end
