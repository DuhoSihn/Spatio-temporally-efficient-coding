function [ W_f, b_f, LS ] = RR_STE_learn_sgm( X, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter, varargin )
% Repetitive Restart optimization of STE_learn_sgm.m
    ...
    while status == 0
        ...
        try
            if iter == 1 && isempty( type_initial )
                if isempty( type_GPU )
                    [ W_f, b_f, tLS ] = STE_learn_sgm( X, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter( iter ), 'params', alpha, beta1, beta2, epsilon );
                else
                    [ W_f, b_f, tLS ] = STE_learn_sgm_GPU( X, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter( iter ), 'params', alpha, beta1, beta2, epsilon );
                end
            elseif ( iter == 1 && ~isempty( type_initial ) ) || iter > 1
                if isempty( type_GPU )
                    [ W_f, b_f, tLS ] = STE_learn_sgm( X, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter( iter ), 'initial', W_f, b_f, 'params', alpha, beta1, beta2, epsilon );
                else
                    [ W_f, b_f, tLS ] = STE_learn_sgm_GPU( X, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter( iter ), 'initial', W_f, b_f, 'params', alpha, beta1, beta2, epsilon );
                end
            end
        catch
            status = 0;
