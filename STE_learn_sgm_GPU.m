function [ W_f, b_f, LS ] = STE_learn_sgm_GPU( X, io_f, hl_f, lambda, kWidth, miniBatchSize, nPred, nMaxIter, varargin )
...
for h = 1 : nH
    for d = 1 : 2
        m_W_f{ h, d } = cell( nP( h ), 1 );
        m_b_f{ h, d } = cell( nP( h ), 1 );
        v_W_f{ h, d } = cell( nP( h ), 1 );
        v_b_f{ h, d } = cell( nP( h ), 1 );
        for p = 1 : nP( h )
...
% -------------------------------------------------------------------------
dW_f_1 = cell( nH, 2 );% gradient for wieght of the first term in Equation ()
db_f_1 = cell( nH, 2 );% gradient for bias of the first term in Equation ()
dW_f_2 = cell( nH, 2 );% gradient for wieght of the first term in Equation ()
db_f_2 = cell( nH, 2 );% gradient for bias of the first term in Equation ()
for h = 1 : nH
    for d = 1 : 2
        dW_f_1{ h, d } = cell( nP( h ), 1 );
        db_f_1{ h, d } = cell( nP( h ), 1 );
        dW_f_2{ h, d } = cell( nP( h ), 1 );
        db_f_2{ h, d } = cell( nP( h ), 1 );
        for p = nP( h )
...
% -------------------------------------------------------------------------
transCoords = cell( nH, 2 );
for h = 1 : nH
    for d = 1 : 2
        if d == 1
            if h < nH
                transCoords{ h, d } = [ [ h + 1, 1 ]; [ h, 2 ] ];
            elseif h == nH
                transCoords{ h, d } = [ [ h, 2 ] ];
            end
        elseif d == 2
            if h > 1
                transCoords{ h, d } = [ [ h, 1 ]; [ h - 1, 2 ] ];
            elseif h == 1
                transCoords{ h, d } = [ [ h, 1 ] ];
            end
        end
...
while stopCriterion == 0
    ...
    if t == 1
        [ Z2, E2, Z1, ~, Z0, oZ1, oZ2 ] = STE_pred_sgm_GPU( repmat( sX, [ 1, 1, nPred ] ), io_f, W_f, b_f, 'learn' );
    else
        for h = 1 : nH
            rX{ h, 1 } = Z2{ h, 1 }( ( nPred - 3 ) * miniBatchSize + [ 1 : miniBatchSize ], : );
        end
        [ Z2, E2, Z1, ~, Z0, oZ1, oZ2 ] = STE_pred_sgm_GPU( repmat( sX, [ 1, 1, nPred ] ), io_f, W_f, b_f, 'learn', 'prior', rX );
    end
    ...
    for h = 1 : nH
        sZ1{ h, 1 } = rand( miniBatchSize * ( nPred - 2 ), nZ( h ), 'gpuArray' );
    end
    ...
    for h = 1 : nH
        ...
        dsk_Z{ h, 1 }( logical( eye( miniBatchSize * ( nPred - 2 ) ) ) ) = NaN;
    ...
    for h = 1 : nH
        ...
        for d2 = [ 1, 2 ]% (2) bottom-up, top-down
            ...
            if d2 == 1
                if h == 1
                    d_cum2_e = zeros( nV, ( nPred - 2 ) * miniBatchSize, 'gpuArray' );
                else
                    d_cum2_e = zeros( nZ( h - 1 ), ( nPred - 2 ) * miniBatchSize, 'gpuArray' );
                end
            elseif d2 == 2
                d_cum2_e = zeros( nZ( h ), ( nPred - 2 ) * miniBatchSize, 'gpuArray' );
            end
    ...
    for h = 1 : nH
        ...
        d_cum_n = 1 ./ ds_Z{ h, 1 };
        d_cum_n( logical( eye( miniBatchSize * ( nPred - 2 ) ) ) ) = NaN;
    ...
    dW_f_t_pool = cell( nH, 2 );
    db_f_t_pool = cell( nH, 2 );
    for h = 1 : nH
        for d = 1 : 2
            dW_f_t_pool{ h, d } = cell( nP( h ), 1 );
            db_f_t_pool{ h, d } = cell( nP( h ), 1 );
            for p = 1 : nP( h )
                ...
                for l = 1 : nL_f( h, d, p )
                    ...
                    if ~( h == 1 && d == 2 )
                        dW_f_t = mean( dW_f_1{ h, d }{ p, 1 }{ l, 1 }, 3 ) + lambda * dW_f_2{ h, d }{ p, 1 }{ l, 1 };% gradient for weight in Equation ()
                        db_f_t = mean( db_f_1{ h, d }{ p, 1 }{ l, 1 }, 2 ) + lambda * db_f_2{ h, d }{ p, 1 }{ l, 1 };% gradient for bias in Equation ()
                    else
                        dW_f_t = mean( dW_f_1{ h, d }{ p, 1 }{ l, 1 }, 3 );% gradient for weight in Equation ()
                        db_f_t = mean( db_f_1{ h, d }{ p, 1 }{ l, 1 }, 2 );% gradient for bias in Equation ()
                    end
    ...
    for h = 1 : nH
        for d = 1 : 2
            if size( invTransCoords{ h, d }, 1 ) == 1
                
                t_h1 = invTransCoords{ h, d }( 1, 1 );
                t_d1 = invTransCoords{ h, d }( 1, 2 );
                
                if d == 1
                    if h == 1
                        t_db_f_t_1 = zeros( nV, 1, 'gpuArray' );
                    else
                        t_db_f_t_1 = zeros( nZ( h - 1 ), 1, 'gpuArray' );
                    end
                elseif d == 2
                    t_db_f_t_1 = zeros( nZ( h ), 1, 'gpuArray' );
                end
                
                for p = 1 : nP( t_h1 )
                    t_db_f_t_1( io_f{ t_h1, t_d1 }{ p, 2 }, 1 ) = t_db_f_t_1( io_f{ t_h1, t_d1 }{ p, 2 }, 1 ) + db_f_t_pool{ t_h1, t_d1 }{ p, 1 }{ nL_f( t_h1, t_d1 ), 1 };
                end
                
                for p = 1 : nP( t_h1 )
                    db_f_t_pool{ t_h1, t_d1 }{ p, 1 }{ nL_f( t_h1, t_d1 ), 1 } = t_db_f_t_1( io_f{ t_h1, t_d1 }{ p, 2 }, 1 );
                end
                
            elseif size( invTransCoords{ h, d }, 1 ) == 2
                
                t_h1 = invTransCoords{ h, d }( 1, 1 );
                t_d1 = invTransCoords{ h, d }( 1, 2 );
                t_h2 = invTransCoords{ h, d }( 2, 1 );
                t_d2 = invTransCoords{ h, d }( 2, 2 );
                
                if d == 1
                    if h == 1
                        t_db_f_t_1 = zeros( nV, 1, 'gpuArray' );
                        t_db_f_t_2 = zeros( nV, 1, 'gpuArray' );
                    else
                        t_db_f_t_1 = zeros( nZ( h - 1 ), 1, 'gpuArray' );
                        t_db_f_t_2 = zeros( nZ( h - 1 ), 1, 'gpuArray' );
                    end
                elseif d == 2
                    t_db_f_t_1 = zeros( nZ( h ), 1, 'gpuArray' );
                    t_db_f_t_2 = zeros( nZ( h ), 1, 'gpuArray' );
                end
                ...
                for p = 1 : nP( t_h2 )
                    t_db_f_t_2( io_f{ t_h2, t_d2 }{ p, 2 }, 1 ) = t_db_f_t_2( io_f{ t_h2, t_d2 }{ p, 2 }, 1 ) + db_f_t_pool{ t_h2, t_d2 }{ p, 1 }{ nL_f( t_h2, t_d2 ), 1 };
                end
                
                t_db_f_t = t_db_f_t_1 + t_db_f_t_2;
                
                for p = 1 : nP( t_h1 )
                    db_f_t_pool{ t_h1, t_d1 }{ p, 1 }{ nL_f( t_h1, t_d1 ), 1 } = t_db_f_t( io_f{ t_h1, t_d1 }{ p, 2 }, 1 );
                end
                for p = 1 : nP( t_h2 )
                    db_f_t_pool{ t_h2, t_d2 }{ p, 1 }{ nL_f( t_h2, t_d2 ), 1 } = t_db_f_t( io_f{ t_h2, t_d2 }{ p, 2 }, 1 );
                end
