...
while stopCriterion == 0
    ...
    if t == 1
        [ Z2, E2, Z1, ~, Z0, oZ1, oZ2 ] = STE_pred_sgm( repmat( sX, [ 1, 1, nPred ] ), io_f, W_f, b_f, 'learn' );
    else
        for h = 1 : nH
            rX{ h, 1 } = Z2{ h, 1 }( ( nPred - 3 ) * miniBatchSize + [ 1 : miniBatchSize ], : );
        end
        [ Z2, E2, Z1, ~, Z0, oZ1, oZ2 ] = STE_pred_sgm( repmat( sX, [ 1, 1, nPred ] ), io_f, W_f, b_f, 'learn', 'prior', rX );
    end
    ...
    for h = 1 : nH
        sZ1{ h, 1 } = rand( miniBatchSize * ( nPred - 2 ), nZ( h ) );
    end
    ...
    for h = 1 : nH
        ...
        for d2 = [ 1, 2 ]% (2) bottom-up, top-down
            ...
            if d2 == 1
                if h == 1
                    d_cum2_e = zeros( nV, ( nPred - 2 ) * miniBatchSize );
                else
                    d_cum2_e = zeros( nZ( h - 1 ), ( nPred - 2 ) * miniBatchSize );
                end
            elseif d2 == 2
                d_cum2_e = zeros( nZ( h ), ( nPred - 2 ) * miniBatchSize );
            end
    ...
    for h = 1 : nH
        for d = 1 : 2
            if size( invTransCoords{ h, d }, 1 ) == 1
                ...
                if d == 1
                    if h == 1
                        t_db_f_t_1 = zeros( nV, 1 );
                    else
                        t_db_f_t_1 = zeros( nZ( h - 1 ), 1 );
                    end
                elseif d == 2
                    t_db_f_t_1 = zeros( nZ( h ), 1 );
                end
            ...
            elseif size( invTransCoords{ h, d }, 1 ) == 2
                ...
                if d == 1
                    if h == 1
                        t_db_f_t_1 = zeros( nV, 1 );
                        t_db_f_t_2 = zeros( nV, 1 );
                    else
                        t_db_f_t_1 = zeros( nZ( h - 1 ), 1 );
                        t_db_f_t_2 = zeros( nZ( h - 1 ), 1 );
                    end
                elseif d == 2
                    t_db_f_t_1 = zeros( nZ( h ), 1 );
                    t_db_f_t_2 = zeros( nZ( h ), 1 );
                end
